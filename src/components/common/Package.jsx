...
const Package = ({packageElemt, isClicked, handleClick}) => {
  ...
  return (
    <div className="col-12 col-sm-6 col-lg-4 px-1 px-sm-2 px-md-3 packages-box" style={{marginBottom: "20px"}}>
      <div 
        className={`p-[30px] px-[10px] rounded-[20px] mb-[20px] flex flex-col justify-between bg-[#e4e2e2] min-h-[500px] max-w-[400px] items-stretch transition-transform 
        ...
        <div className="package-first-part">
          ...
          {packageElemt.detail && <p style={{color: "black"}}>{packageElemt.detail}</p>}
        ...
        <div className="package-elements-container">
          <ul>
            {packageElemt.elements.map((elmt, idx) => (
              <li key={idx} className="text-lg leading-7">{elmt}</li>
            ))}
          </ul>
        </div>
